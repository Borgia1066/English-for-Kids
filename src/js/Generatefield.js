...
import { playMode, clearScore } from './Gmode';

const cardsData = {array: [], currentCard: null,isStatsOn: false,};
const playButton = document.querySelector('.play_btn');
const switchButton = document.querySelector('.switch_btn');
const repeatButton = document.querySelector('.repeat');
const categoryName = document.querySelector('.category-name');
const stats = document.querySelector('.stats_btn');
    ...
    if (!e.target.classList.contains('wrapper')) {
      ...
      if (clickedCard.getAttribute('data-category') === 'main') {
        if (cardsData.currentCard) {
          ...
          categoryName.innerText = cardsData.currentCard.toUpperCase();
          refreshField();

          cardsData.array = addCardsToDom(cardsData.currentCard);
          nav.childNodes.forEach((category) => {
            let currentCategory = category.innerText.toLowerCase();
...
export {generateCards, addCardsToDom, chooseCategory, refreshField, cardsData,};
