...
export default class Card {
  ...
  generateCard() {
    ...
    if (this.word !== 'milky way') {
      this.card.setAttribute('data-name', this.word.toLowerCase());
    } else {
      this.card.setAttribute('data-name', this.word.toLowerCase());
    }
  ...
  addEventListenersToCard() {
    ...
    this.cardFrontSide.addEventListener('click', (e) => {
      if (!e.target.classList.contains('card__button') && !Play.isPlaying) this.playAudio();
      ...
      this.clicks += 1;
      if (!Play.isPlaying) localStorage.setItem(name, this.clicks);
    });
  }
